...
//installing shadcn dialog from shadcn and then importing it
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const ShareModal = ({roomId,collaborators,creatorId,currentUserType}:ShareDocumentDialogProps) => {
    //we want to check which user is trying to share the document
    //aka change the permisson
    const user=useSelf();
            ...
            <DialogContent className='shad-dialog'>
                ...
                <div className='flex item-center gap-3'>
                    <div className='flex flex-1 rounded-md bg-dark-400'>
                        ...
                        //this is to set the initial "value" of the input field
                        value={email} 
                        onChange={(e)=>setEmail(e.target.value)}
                        className='share-input'/> 
                        <UserTypeSelector userType={userType} setUserType={setUserType}/>
                    </div>
                    <Button type="submit" onClick={()=>shareDocumentHandler(userType)}
                    className='gradient-blue flex h-full gap-1 px-5' disabled={loading}>
                        {loading ?'Sending...':'Invite'}
                    </Button>
                </div> 
                <div className='m-y-2 space-y-2'>
